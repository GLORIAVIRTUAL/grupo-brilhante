...
// ============================================================================
// SENDER EXCLUSIVO DA 2ª CONEXÃO Z-API (Loja Moinhos Shopping).
// É idêntico ao zapi_sender, mas usa as credenciais da instância de MOINHOS
// (ZAPI_MOINHOS_*), garantindo que as respostas saiam pelo número certo.
// ============================================================================
Deno.serve(async (req) => {
    try {
        ...
        if (isGroupTarget) {
            console.warn('Blocked outbound WhatsApp group target (Moinhos).');
        ...
        if (normalizedPhone.length < 10 || normalizedPhone.length > 15) throw new SecurityError('Número de WhatsApp inválido.', 400, 'INVALID_PHONE');
        if (!['TEXT', 'IMAGE', 'AUDIO', 'DOC', 'BUTTONS', 'OPTION_LIST'].includes(type)) throw new SecurityError('Tipo de mensagem inválido.', 400, 'INVALID_MESSAGE_TYPE');
        ...
        if (type === 'BUTTONS' && (!Array.isArray(buttons) || buttons.length < 1 || buttons.length > 3 || buttons.some((button) => !String(button?.label || '').trim() || String(button.label).length > 80))) throw new SecurityError('Lista de botões inválida.', 400, 'INVALID_BUTTONS');
        if (type === 'OPTION_LIST' && (!Array.isArray(optionList?.options) || optionList.options.length < 1 || optionList.options.length > 10)) throw new SecurityError('Lista de opções inválida.', 400, 'INVALID_OPTION_LIST');
        ...
        const INSTANCE_ID = Deno.env.get("ZAPI_MOINHOS_INSTANCE_ID");
        const TOKEN = Deno.env.get("ZAPI_MOINHOS_TOKEN");
        const CLIENT_TOKEN = Deno.env.get("ZAPI_MOINHOS_SECURITY_TOKEN");

        if (!INSTANCE_ID || !TOKEN || !CLIENT_TOKEN) throw new SecurityError('Integração Z-API Moinhos não configurada.', 503, 'ZAPI_MOINHOS_NOT_CONFIGURED');
        ...
        if (type === 'IMAGE') {
            ...
            payload = { phone: normalizedPhone, message, optionList, delayMessage: 2 };
        }

        let zapiResponseData = null;

        {
            console.log('Sending via Z-API Moinhos', { type, has_conversation: Boolean(finalConversationId), unit_id: targetUnitId });
            ...
            const sendRequest = async (url, requestPayload) => {
                ...
                if (!response.ok) throw new Error(`Z-API request failed (${response.status})`);
            ...
            try {
                ...
                if (!isInteractive) {
                    console.error('Z-API Error (Moinhos):', error.message);
                    throw error;
                }
                console.warn('Interactive message failed (Moinhos), retrying as plain text:', error.message);
        ...
        // Log da mensagem na conversa Moinhos do cliente, quando houver.
        if (!finalConversationId && targetCustomer) {
            const conversations = await base44.asServiceRole.entities.Conversation.filter({
                customer_id: targetCustomer.id,
                channel: 'WHATSAPP'
            });
            const moinhosConv = conversations.find(c => (c.metadata || {}).source === 'zapi_moinhos');
            if (moinhosConv) finalConversationId = moinhosConv.id;
        }
        ...
        if (finalConversationId) {
            ...
            newMessage = await base44.asServiceRole.entities.Message.create({
                ...
                raw_payload: { ...zapiResponseData, source: 'zapi_moinhos' }
            });
        ...
        console.error("Error in zapi_moinhos_sender:", error?.code || error?.message || error);
