  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        onSubmit({ implementation_status: stage, reason });
        setReason('');
      }}
    >
      ...
      <label className="block space-y-1.5">
        <span className="text-xs font-medium uppercase tracking-wide text-white/55">Justificativa</span>
        <input className={inputClass} value={reason} onChange={(event) => setReason(event.target.value)} minLength={8} required />
      </label>
