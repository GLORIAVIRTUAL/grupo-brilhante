...
export default function StockOperationDialog({ open, onOpenChange, stockItems = [], units = [], selectedUnitId, batches = [], initialAction = 'adjust_in', initialItemId = '', onCompleted }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-2xl border-white/10 bg-[#17364F] text-white"><DialogHeader><DialogTitle>Operação de estoque</DialogTitle><DialogDescription className="text-white/45">Saldos e custos são alterados somente pelo servidor e ficam auditados.</DialogDescription></DialogHeader>
    <div className="grid gap-2 sm:grid-cols-5">{ACTIONS.map(({ value, label, icon: Icon }) => <button key={value} type="button" onClick={() => setAction(value)} className={`rounded-2xl border p-3 text-left text-xs transition ${action === value ? 'border-[#216FA1] bg-[#216FA1]/15 text-white' : 'border-white/10 bg-white/[0.03] text-white/50 hover:bg-white/[0.06]'}`}><Icon className="mb-2 h-4 w-4" />{label}</button>)}</div>
    {action === 'create_item' ? <div className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="sm:col-span-2"><Label>Insumo</Label><select value={form.stock_item_id} onChange={(e) => patch('stock_item_id', e.target.value)} className="mt-1 h-10 w-full rounded-md border border-white/10 bg-[#10243a] px-3 text-sm"><option value="">Selecione</option>{scoped.map((item) => <option key={item.id} value={item.id}>{item.name} · {Number(item.current_quantity || 0).toLocaleString('pt-BR')} {item.base_unit}</option>)}</select></div>
