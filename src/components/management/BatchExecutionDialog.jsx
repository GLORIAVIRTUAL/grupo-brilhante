...
export default function BatchExecutionDialog({ open, onOpenChange, batch, laborEntries = [], stockItems = [], onCompleted }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-4xl border-white/10 bg-[#17364F] text-white"><DialogHeader><DialogTitle>{batch.code}</DialogTitle><DialogDescription className="text-white/45">Execução rastreada · {batch.piece_count || batch.garment_item_ids?.length || 0} peças · {batch.machine_id || 'etapa manual'}</DialogDescription></DialogHeader>
    <div className="grid gap-4 md:grid-cols-4"><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><p className="text-xs text-white/40">Estado</p><Badge className="mt-2 bg-[#216FA1]/20 text-blue-200">{STATUS[batch.status] || batch.status}</Badge></div><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><p className="text-xs text-white/40">Capacidade</p><p className="mt-2 text-xl font-bold text-white">{Number(batch.capacity_percent || 0).toFixed(1)}%</p></div><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><p className="text-xs text-white/40">Previsto</p><p className="mt-2 text-xl font-bold text-cyan-300">{money(batch.estimated_material_cost)}</p></div><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><p className="text-xs text-white/40">Real</p><p className="mt-2 text-xl font-bold text-emerald-300">{money(batch.total_actual_cost || batch.actual_material_cost)}</p></div></div>
