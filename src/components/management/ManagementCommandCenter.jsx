...
export default function ManagementCommandCenter({ selectedUnitId, defaultUnitId, customers = [], onManualEntry, onManualQuote }) {
  ...
  return (
    <section className="space-y-6">
      <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-br from-[#17364F] via-[#10243a] to-[#10243a] p-6 shadow-2xl shadow-blue-950/20 md:p-8">
        <div className="absolute -right-16 -top-24 h-64 w-64 rounded-full bg-[#216FA1]/20 blur-3xl" /><div className="absolute -bottom-24 left-1/3 h-64 w-64 rounded-full bg-orange-500/10 blur-3xl" />
        <div className="relative flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl"><div className="flex items-center gap-2 text-sm font-medium text-blue-200"><Sparkles className="h-4 w-4" />Operação assistida por inteligência</div><h2 className="mt-2 text-2xl font-bold text-white md:text-3xl">Centro de comando da lavanderia</h2><p className="mt-2 text-sm leading-relaxed text-white/48 md:text-base">Registre peças por foto, leia notas e contas, acompanhe cada roupa e trate somente as exceções que exigem decisão humana.</p></div>
          <div className="flex flex-wrap gap-2"><Button onClick={() => setSmartQuoteOpen(true)} className="bg-gradient-to-r from-[#216FA1] to-[#2d8ac4]"><Camera className="mr-2 h-4 w-4" />Orçamento por fotos</Button><Button variant="outline" onClick={onManualQuote} className="border-white/15 bg-white/5"><Plus className="mr-2 h-4 w-4" />Orçamento manual</Button></div>
        </div>
        <div className="relative mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {[
            ['Peças em operação', summary.activeGarments, Shirt, 'text-sky-300'], ['Atrasos', summary.overdue, TriangleAlert, 'text-red-300'], ['Revisões', summary.pendingReviews, ClipboardCheck, 'text-blue-300'], ['Estoque baixo', summary.lowStock, Boxes, 'text-amber-300'], ['Contas abertas', summary.pendingPayables, Banknote, 'text-emerald-300'],
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <ActionCard icon={Camera} title="Fotos das peças" description="Identificar, relacionar ao catálogo e calcular o orçamento." accent="from-[#216FA1] to-[#2d8ac4]" onClick={() => setSmartQuoteOpen(true)} />
        ...
        {canManageBilling && <ActionCard icon={Handshake} title="Convênios" description="Limites, pedidos faturados e fechamentos periódicos." accent="from-[#216FA1] to-[#2d8ac4]" onClick={() => { setWorkspaceTab('commercial'); document.getElementById('management-workspace')?.scrollIntoView({ behavior: 'smooth' }); }} />}
        {canManageQuotes && <ActionCard icon={FileClock} title="Ciclo do orçamento" description="Versões, validade, envio, aceite e cancelamento auditado." accent="from-[#216FA1] to-[#2d8ac4]" onClick={() => { setWorkspaceTab('commercial'); document.getElementById('management-workspace')?.scrollIntoView({ behavior: 'smooth' }); }} />}
        ...
        <ActionCard icon={ShieldCheck} title="Revisões" description="Centralizar baixa confiança e decisões sensíveis." accent="from-[#216FA1] to-[#2d8ac4]" badge={summary.pendingReviews} onClick={() => { setWorkspaceTab('reviews'); document.getElementById('management-workspace')?.scrollIntoView({ behavior: 'smooth' }); }} />
      </div>
      ...
      <Tabs id="management-workspace" value={workspaceTab} onValueChange={setWorkspaceTab} className="space-y-5 scroll-mt-6">
        <TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto rounded-2xl bg-white/5 p-1.5">
          <TabsTrigger value="operations" className="gap-2 whitespace-nowrap data-[state=active]:bg-[#216FA1] data-[state=active]:text-white"><Gauge className="h-4 w-4" />Operação</TabsTrigger>
          <TabsTrigger value="custody" className="gap-2 whitespace-nowrap data-[state=active]:bg-[#216FA1] data-[state=active]:text-white"><MapPin className="h-4 w-4" />Etiquetas e entrega</TabsTrigger>
          {canManageProduction && <TabsTrigger value="production" className="gap-2 whitespace-nowrap data-[state=active]:bg-[#216FA1] data-[state=active]:text-white"><Factory className="h-4 w-4" />Produção</TabsTrigger>}
          {canManageInventory && <TabsTrigger value="inventory" className="gap-2 whitespace-nowrap data-[state=active]:bg-[#216FA1] data-[state=active]:text-white"><Boxes className="h-4 w-4" />Estoque</TabsTrigger>}
          {canViewInsights && <TabsTrigger value="insights" className="gap-2 whitespace-nowrap data-[state=active]:bg-[#216FA1] data-[state=active]:text-white"><Activity className="h-4 w-4" />Indicadores</TabsTrigger>}
          {canViewFinancial && <TabsTrigger value="financial" className="gap-2 whitespace-nowrap data-[state=active]:bg-[#216FA1] data-[state=active]:text-white"><Banknote className="h-4 w-4" />Recebimentos e caixa</TabsTrigger>}
          {(canManageQuotes || canManageBilling) && <TabsTrigger value="commercial" className="gap-2 whitespace-nowrap data-[state=active]:bg-[#216FA1] data-[state=active]:text-white"><Handshake className="h-4 w-4" />Orçamentos e faturados</TabsTrigger>}
          {canManageFiscal && <TabsTrigger value="fiscal" className="gap-2 whitespace-nowrap data-[state=active]:bg-[#216FA1] data-[state=active]:text-white"><Landmark className="h-4 w-4" />Fiscal</TabsTrigger>}
          <TabsTrigger value="reviews" className="gap-2 whitespace-nowrap data-[state=active]:bg-[#216FA1] data-[state=active]:text-white"><ClipboardCheck className="h-4 w-4" />Revisões {summary.pendingReviews > 0 && <Badge className="ml-1 bg-orange-500 text-white">{summary.pendingReviews}</Badge>}</TabsTrigger>
        </TabsList>
