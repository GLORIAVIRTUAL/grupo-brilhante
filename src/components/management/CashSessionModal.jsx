...
export default function CashSessionModal({ open, onOpenChange, unitId, sessions = [], onProcessed }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => !busy && onOpenChange(value)}>
      <DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto border-white/10 bg-[#17364F] text-white">
        ...
        {!activeSession ? (
          ...
          <div className="space-y-5 py-3">
            ...
            <div className="space-y-3 rounded-3xl border border-white/10 bg-white/[0.03] p-4"><div className="flex items-center gap-2"><Scale className="h-4 w-4 text-blue-300" /><h3 className="font-semibold">Conferência e fechamento</h3></div><div className="space-y-2"><Label>Dinheiro contado</Label><Input type="number" min="0" step="0.01" value={countedAmount} onChange={(event) => setCountedAmount(event.target.value)} className="border-white/10 bg-black/20" /></div>{countedAmount !== '' && Math.abs(difference) >= 0.01 && <div className="space-y-2"><Label>Motivo da diferença de {money(difference)}</Label><Input value={differenceReason} onChange={(event) => setDifferenceReason(event.target.value)} className="border-amber-500/30 bg-amber-500/5" /></div>}<Button disabled={busy || countedAmount === '' || (Math.abs(difference) >= 0.01 && differenceReason.trim().length < 8)} onClick={() => run({ action: 'close', cash_session_id: activeSession.id, counted_cash_amount: Number(countedAmount), difference_reason: differenceReason }, 'Fechamento registrado.')} className="w-full bg-[#216FA1] hover:bg-[#2d8ac4]">{busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Check className="mr-2 h-4 w-4" />}Conferir e fechar</Button></div>
          </div>
        )}
      </DialogContent>
    </Dialog>
