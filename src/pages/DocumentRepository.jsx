...
export default function DocumentRepository() {
  ...
  const [form, setForm] = useState({ legal_entity_id: '', unit_id: '', document_type: 'corporate_document', classification: 'internal', reason: autoReason('Armazenamento de documento') });
  const [file, setFile] = useState(null);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);
  const [notice, setNotice] = useState(null);
  const [retire, setRetire] = useState({ asset_id: '', reason: autoReason('Aposentadoria de documento') });
  ...
  const upload = async (event) => {
    ...
    try {
      ...
      setFile(null);
      setForm((current) => ({ ...current, reason: autoReason('Armazenamento de documento') }));
  ...
  const retireAsset = async () => {
    ...
    try {
      ...
      setRetire({ asset_id: '', reason: autoReason('Aposentadoria de documento') });
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
        ...
        <div className="overflow-hidden rounded-xl border border-white/10">
          ...
          {assets.map((asset) => <div key={asset.id} className="grid gap-2 border-t border-white/8 px-4 py-3 first:border-t-0 md:grid-cols-[1.6fr_1fr_1fr_.7fr_auto] md:items-center md:gap-3"><div><p className="truncate text-sm font-medium text-white">{asset.original_filename || asset.safe_filename || asset.id}</p><p className="mt-1 text-xs text-white/35">{Math.max(1, Math.round(Number(asset.size_bytes || 0) / 1024))} KB · retenção {asset.retention_class || 'standard'}</p></div><span className="text-sm text-white/60">{labelOf(asset.document_type)}</span><span className="text-sm text-white/60">{asset.classification || 'internal'}</span><span className="inline-flex w-fit items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-2 py-1 text-xs text-white/55"><FileCheck2 className="h-3.5 w-3.5" />{asset.validation_status || 'pending'}</span>{canManage && (asset.status || 'active') !== 'retired' ? <button type="button" onClick={() => setRetire({ asset_id: asset.id, reason: autoReason('Aposentadoria de documento') })} className="inline-flex items-center gap-1.5 text-xs text-amber-200 hover:text-amber-100"><Archive className="h-3.5 w-3.5" />Aposentar</button> : <span className="text-xs text-white/30">{asset.status || 'active'}</span>}</div>)}
