...
export default function EnterpriseFinance() {
  ...
  const [period, setPeriod] = useState({ year: today.getFullYear(), month: today.getMonth() + 1, reason: autoReason('Abertura de competência') });
  const [projection, setProjection] = useState({ description: '', direction: 'inflow', amount: '', probability_percent: 100, projection_date: '', scenario: 'baseline', reason: autoReason('Projeção financeira') });
  const [intercompany, setIntercompany] = useState({ destination_legal_entity_id: '', source_unit_id: '', destination_unit_id: '', source_business_party_id: '', destination_business_party_id: '', description: '', competence_date: '', due_date: '', amount: '', reason: autoReason('Lançamento intercompany') });
  const actionReason = autoReason('Ação financeira');
  ...
  const createProjection = (event) => {
    ...
    run(async () => {
      ...
      setProjection((current) => ({ ...current, description: '', amount: '' }));
    }, 'Projeção registrada.');
  };
  ...
  const createIntercompany = (event) => {
    ...
    run(async () => {
      ...
      setIntercompany((current) => ({ ...current, description: '', amount: '' }));
