...
export default function BusinessRegistry() {
  ...
  const [form, setForm] = useState({ party_type: 'company', legal_name: '', trade_name: '', tax_id: '', segment: 'other', role_type: 'customer', unit_id: '', reason: '' });
  const [contact, setContact] = useState({ contact_type: 'whatsapp', contact_name: '', value: '', is_primary: true, allows_marketing: false, reason: '' });
  const [address, setAddress] = useState({ address_type: 'operational', postal_code: '', street: '', number: '', neighborhood: '', city: '', state: '', reason: '' });
  ...
  const createParty = async (event) => {
    ...
    try {
      ...
      setForm((current) => ({ ...current, legal_name: '', trade_name: '', tax_id: '', reason: '' }));
